...
class FilmsViewController: UIViewController {
    ...
    private func startLoading() {
        ...
        showFilmsContent(false)
    }
    
    private func stopLoading() {
        activityIndicatorView.stopAnimating()
        activityIndicatorView.removeFromSuperview()
        showFilmsContent(true)
    }
    ...
    private func showErrorView(with message: String, andTitle title: String) {
        view.addSubview(errorView)
        errorView.configure(with: message, andTitle: title)
    ...
    private func showFilmsContent(_ showContent: Bool) {
        filmsContentView.isHidden = !showContent
    }
    
    private func renderError(with message: String, andTitle title: String) {
        stopLoading()
        UIView.animate(withDuration: 0.1) {
            self.showFilmsContent(false)
        }
        searchController.searchBar.isHidden = true
        showErrorView(with: message, andTitle: title)
    }
    
    private func renderSuccess(with mostRatedSnapshot: MostRatedFilmsSnapshot, and posterFilmSnapshot: PosterFilmsSnapshot) {
        if mostRatedSnapshot.numberOfItems != .zero &&
            posterFilmSnapshot.numberOfItems != .zero {
            stopLoading()
            showFilmsContent(true)
    ...
    private func renderSeach(with posterFilmsSnapshot: PosterFilmsSnapshot) {
        if posterFilmsSnapshot.numberOfItems != .zero {
            showFilmsContent(false)
            showSearchedFilms()
    ...
    private func render(_ state: FilmsViewControllerState) {
        switch state {
        case .error(let message, let title):
            renderError(with: message, andTitle: title)
        case .loading:
            startLoading()
...
extension FilmsViewController: UISearchBarDelegate {
    func searchBarTextDidBeginEditing(_ searchBar: UISearchBar) {
        ...
        UIView.animate(withDuration: 0.2) {
            ...
            self.viewModel.searchBarDidBeginEditing()
        }
    ...
    func searchBarCancelButtonClicked(_ searchBar: UISearchBar) {
        ...
        stopLoading()
        UIView.animate(withDuration: 0.1) {
            self.showFilmsContent(true)
        }
    }
}

extension FilmsViewController: ErrorViewDelegate {
